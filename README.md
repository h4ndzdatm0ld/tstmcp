...
const response = await client.config.create().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: config, response: raw } = await client.config.create().withResponse();
...
1. Via the `DEVKNOT_LOG` environment variable
...
const client = new Devknot({
  logger: logger.child({ name: 'Devknot' }),
...
const client = new Devknot({ fetch });
```
...
```ts
import Devknot from 'npm:devknot';
